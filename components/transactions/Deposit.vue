    ...
    <div >
       <div>
        <table class="w-full text-black/60 overflow-scroll font-sans bg-slate-100">
            ...
            <tr v-for="data, index in depositTransactions.slice(0, limit=='all'?depositTransactions.length:limit)" class="h-10 text-center text-base font-semibold border">
...
function filterDeposit() {
    for (const transaction of props.trs) {
        if (transaction.type == 'deposit') {
            depositTransactions.value.push(transaction)
        }
    }
}
